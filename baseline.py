...
class WideAndDeep(object):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, stage, action):
        ...
        self.num_epochs_dict = {"read_comment": 3, "like": 3, "click_avatar": 3, "favorite": 3, "forward": 3,
                                "comment": 3, "follow": 3}
        self.estimator = None
    ...
    def df_to_dataset(self, df, stage, action, shuffle=True, batch_size=256, num_epochs=3):
        '''
    ...
    def input_fn_predict(self, df, stage, action):
        return self.df_to_dataset(df, stage, action, shuffle=False, batch_size=len(df), num_epochs=3)
...
def get_feature_columns():
    ...
    crossed_columns = [
        tf.feature_column.crossed_column(['userid','feedid',"staysum","playsum"],hash_bucket_size = 1000),
        tf.feature_column.crossed_column(['userid','authorid','bgm_song_id',"staysum","playsum"], hash_bucket_size=1000),
    ]
    ...
    linear_feature_columns = linear_feature_columns + crossed_columns
    # 行为统计特征
